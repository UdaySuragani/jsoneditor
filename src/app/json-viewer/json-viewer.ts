...
export class JsonViewer {
  ...
  formatJson(): void {
    try {
      ...
      // Parse JSON to validate and get clean data
      const jsonData = JSON.parse(textarea.value);
      
      // Format the JSON properly
      const formattedJson = JSON.stringify(jsonData, null, 2);
      
      // Update the textarea with cleaned, formatted JSON
      textarea.value = formattedJson;
      
      // Parse and update the edit lines
      this.parseEditLines(jsonData);
      this.hasUnsavedChanges.set(true);
      this.validationError.set('');
      this.isValidJson.set(true);
      this.lastTextareaLineCount = formattedJson.split('\n').length;
    } catch (error) {
      if (error instanceof Error) {
